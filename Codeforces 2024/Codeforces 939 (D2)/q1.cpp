...
//..........................................
int reduce(int ak[], int ans, int k){
    for(int j=0;j<k;j++){
        if(ak[j]<=ans){
            ans--;
        }
    ...
    if(ans>=ak[0]){
        return reduce(ak,ans,k); // return the result of recursive call
    }
    else{
        return ans;
    }
...
void solve(){
    ...
    int aq[q];
    for(int i=0;i<q;i++){
        cin>>aq[i]; // fix the indexing here
    }
    for(int i=0;i<q;i++){ // iterate over the aq array correctly
        int ans=aq[i];
        int result = reduce(ak,ans,k);
        cout<<result<<" ";
    }
    cout<<endl;
}
