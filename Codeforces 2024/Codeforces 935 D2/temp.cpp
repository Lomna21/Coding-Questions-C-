...
void solve(){
    int n;
        ...
        int bestPosition = -1, minDistance = INT_MAX;

        for (int k = 1; k < n; ++k) {
            int left = k;
            int right = n - k;
            int leftSatisfied = count(s.begin(), s.begin() + k, '0');
            int rightSatisfied = count(s.begin() + k, s.end(), '1');

            // Prioritize positions where all residents on one side are satisfied
            if (leftSatisfied == left || rightSatisfied == right) {
                int distance = abs(n / 2 - k);
                if (distance < minDistance) {
                    bestPosition = k;
                    minDistance = distance;
                }
            } else if (leftSatisfied >= ceil(left / 2.0) && rightSatisfied >= ceil(right / 2.0)) {
                int distance = abs(n / 2 - k);
                if (distance < minDistance || (distance == minDistance && k < bestPosition)) {
                    bestPosition = k;
                    minDistance = distance;
                }
            }
        ...
        cout << (bestPosition == -1 ? 0 : bestPosition) << endl;
