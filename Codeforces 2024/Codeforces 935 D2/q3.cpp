...
#include<bits/stdc++.h>
using namespace std;
#define fast ios_base::sync_with_stdio(false);cin.tie(NULL);std::cout.tie(NULL)
...
void solve(){
    ...
    // Outputting the count of '1's at each index
    for (int i = 0; i <= n; i++) {
        //std::cout << v[i] << " ";
    }
    //std::cout << endl;

    // Finding the index with approximately half '1's on both sides
    for (int i = 1; i <= n; i++) {
        if (v[i] * 2 <= i && (v[n] - v[i]) * 2 >= n - i) {
            std::cout << i << endl;
            return;
        }
    }
