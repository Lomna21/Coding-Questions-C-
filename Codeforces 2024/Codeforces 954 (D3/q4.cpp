...
// Function to perform the arithmetic operation based on the given operator
long long calculate(long long a, long long b, char op) {
    if (op == '+') return a + b;
    else return a * b;
}

// Recursive function to find the minimum result of the expression
long long findMinValue(string s, int pos, int cnt) {
    if (cnt == 0) return stoll(s.substr(pos));
    if (dp[pos][cnt] != -1) return dp[pos][cnt];
    
    long long result = LLONG_MAX;
    for (int i = pos; i < s.size() - cnt; i++) {
        long long a = stoll(s.substr(pos, i - pos + 1));
        long long b = findMinValue(s, i + 1, cnt - 1);
        result = min(result, calculate(a, b, '+'));
        result = min(result, calculate(a, b, '*'));
    }
    return dp[pos][cnt] = result;
}
...
void solve() {
    ...
    cout << findMinValue(s, 0, n - 2) << endl;
}
signed main(){
    fast;
    int T;cin>>T;
